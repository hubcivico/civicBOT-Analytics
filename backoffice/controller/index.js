...
function imageFormatter(value, row) {
  if(row.photo!=null){
    ...
    return '<p><strong>"'+row.text+'"</strong></p>'
  }
}
...
function detailFormatter(index, row) {
  ...
  if (row.photo != null) {
    ...
    html.push('</td><td><p><strong>"' + row.text + '"</strong></p></td></tr></table>');
  }
...
window.operateEvents = {
  'click .save': function(e, value, row, index) {
    ...
    setMediaName(row.id, row.media.name);
    setMediaId(row.id, row.media.id);
  },
